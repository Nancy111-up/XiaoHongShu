...
`DASHSCOPE_API_KEY` 优先于 `LLM_API_KEY`。缺少有效 API key 时，若采集证据已进入分析阶段，刷新会以“部分完成”结束并保留数据；若没有证据，则可正常完成但产生零机会。系统不会编造机会或草稿。
...
- **完成**：搜索、详情采集和机会分析均完成。
- **部分完成**：至少有一部分数据被安全保留，例如某些关键词或详情采集失败，或者已有采集证据进入分析时 AI 未配置/不可用。若本次没有采集到证据，任务也可能正常完成但机会数为零。查看界面中的失败关键词和安全摘要，修复后再次刷新。
...
- 机会分析和接受机会后的完整草稿生成需要有效的 AI 配置。AI 不可用不会丢弃已采集内容，也不会产生伪造的机会。
- 工作区已装配“机会 → 草稿 → 日历 → 复盘”的界面、API 和内容服务。配置有效 AI Key 后可接受机会并生成草稿；拒绝和已有草稿排期也使用同一持久化服务。

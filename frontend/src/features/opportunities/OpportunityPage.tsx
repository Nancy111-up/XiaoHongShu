...
import { BrandBrain } from "../workspace/BrandBrain"
...
export function OpportunityPage() {
  ...
  return <main className="workspace-shell fresh-forest" data-testid="app-shell">
    ...
    <section className="app-area">
      ...
      {activeModule===4 && <BrandBrain/>}
      {activeModule===0 && (loading ? <StatePanel text="正在读取最新机会…"/> : error ? <StatePanel text={error}/> : !selected ? <StatePanel text="还没有可展示的机会，请先刷新热点。"/> : <Workspace items={items} selected={selected} onSelect={setSelectedId} onNotice={setNotice} onDraftCreated={()=>setDraftCount(count=>count+1)}/>)}
    </section>
  </main>
}
